...
function ensureFloatingBag(){let bag=document.getElementById('floatingBag');if(bag)return bag;bag=document.createElement('a');bag.id='floatingBag';bag.className='floating-bag';bag.href='cart.html';bag.setAttribute('aria-label','Buka keranjang');bag.innerHTML='<span class="floating-bag-icon">◰</span><span><b class="floating-bag-title">BAG</b><small class="floating-bag-meta"><span data-cart-count>0</span> ITEM · VIEW BAG</small></span><span class="floating-bag-arrow">↗</span>';document.body.appendChild(bag);return bag}
function addCart(id,size='M',qty=1){const c=getCart(),p=byId(id);if(!p)return;const key=id+'-'+size,x=c.find(i=>i.key===key);if(x)x.qty+=qty;else c.push({key,id,size,qty,name:p.name,price:p.price,img:p.images[0],preorder:p.preorder});setCart(c);ensureFloatingBag().classList.add('show');toast('Ditambahkan ke keranjang')}
