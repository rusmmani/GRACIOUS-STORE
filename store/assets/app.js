...
async function nav(){injectStoreControls();document.querySelectorAll('[data-cart]').forEach(e=>{e.classList.add('store-cart-link');e.innerHTML='<span aria-hidden="true">◰</span><span class="cart-word">'+t('bag')+'</span><span class="cart-count" data-cart-count>0</span>';});updateCartCount();const l=document.querySelector('.loader');window.addEventListener('load',()=>setTimeout(()=>l?.classList.add('hide'),320));await Promise.all([loadCatalog(),loadStoreProfile(),loadStoreMode()]);setupButtonMotion();applyStorePreferences()}
