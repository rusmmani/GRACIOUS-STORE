...
create table if not exists public.product_reviews(
  ...
  reviewer_name text not null default 'Anonymous', rating integer not null check(rating between 1 and 5),
  comment text not null default '', photo_url text, created_at timestamptz not null default now()
);
create index if not exists product_reviews_product_id_idx on public.product_reviews(product_id);
alter table public.product_reviews enable row level security;
drop policy if exists gracious_reviews_select on public.product_reviews;
create policy gracious_reviews_select on public.product_reviews for select to anon,authenticated using(true);
drop policy if exists gracious_reviews_insert on public.product_reviews;
create policy gracious_reviews_insert on public.product_reviews for insert to anon,authenticated with check(rating between 1 and 5 and length(trim(reviewer_name)) between 1 and 60 and length(comment) <= 1000);
drop policy if exists gracious_reviews_delete on public.product_reviews;
create policy gracious_reviews_delete on public.product_reviews for delete to authenticated using(public.is_admin());
insert into storage.buckets(id,name,public) values('review-images','review-images',true) on conflict(id) do nothing;
drop policy if exists gracious_review_image_upload on storage.objects;
create policy gracious_review_image_upload on storage.objects for insert to anon,authenticated with check(bucket_id='review-images');
drop policy if exists gracious_review_image_select on storage.objects;
create policy gracious_review_image_select on storage.objects for select to public using(bucket_id='review-images');
drop policy if exists gracious_review_image_delete on storage.objects;
create policy gracious_review_image_delete on storage.objects for delete to authenticated using(bucket_id='review-images' and public.is_admin());
