...
async function supa(){if(!window.supabase||!window.GRACIOUS_CONFIG)return null;return window.supabase.createClient('https://dlkdtmdmauqvbumhyqsu.supabase.co','sb_publishable_A5Lenr4D9XPyVTHgoIozuA_9wfsvfIc')}
...
async function loadCatalog(){try{const sb=await supa();if(!sb)return;const [{data:p},{data:c},{data:pay}]=await Promise.all([sb.from('products').select('*,categories(name,slug)').eq('active',true).order('created_at',{ascending:false}),sb.from('categories').select('*').eq('active',true).order('name'),sb.from('payment_methods').select('*').eq('active',true).order('sort_order').order('created_at')]);if(p?.length)PRODUCTS=p.map(normalizeProduct);if(c)CATEGORIES=c;if(pay)PAYMENT_METHODS=pay}catch(e){console.warn('Catalog unavailable',e)}return PRODUCTS}
